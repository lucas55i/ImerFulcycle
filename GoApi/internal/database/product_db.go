...
import (
	...
	"github.com/imerfullcycle/goapi/internal/entity"
)
...
func (pd *ProductDB) GetProductByCategoryID(categoryID string) ([]*entity.Product, error) {
	...
	for rows.Next() {
		var product entity.Product
		if err := rows.Scan(&product.ID, &product.Name, &product.CategoryID, &product.Description, &product.ImageURL, &product.Price); err != nil {
			return nil, err
		}
		products = append(products, &product)
	}
...
func (pd *ProductDB) CreateProduct(product *entity.Product) (*entity.Product, error) {
	_, err := pd.db.Exec("INSERT INTO products (id,name) VALUES (?, ?, ? ,?, ? )", product.ID, product.Name, product.Description, product.Price, product.ImageURL)
	if err != nil {
		return nil, err
	}
	return product, nil
